resource "snowflake_view" "RDV_MDM_VW_QUALITY_SAT_MDM_PARTY" {
  database = local.databases[var.DATABASE]
  schema = snowflake_schema.RDV_MDM.name
  name = "VW_QUALITY_SAT_MDM_PARTY"

  copy_grants = true
  statement = <<-EOT
with cte_last_version as(
    SELECT *
    FROM ${local.databases["DWH"]}.RDV_MDM.VW_SAT_MDM_PARTY
...
FROM cte_last_version a
    LEFT JOIN ${local.databases["DWH"]}.RDV_MDM.HUB_MDM_PARTY b
...
depends_on = [
    snowflake_table.RDV_MDM_HUB_MDM_PARTY,
    snowflake_view.RDV_MDM_VW_SAT_MDM_PARTY
  ]
